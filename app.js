...
async function barberSignup() {
  ...
  try {
    ...
    utils.showAlert('تم إنشاء الحساب بنجاح!');
...
async function barberLogin() {
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      utils.showAlert('تم تسجيل الدخول بنجاح!');
    } else {
...
async function bookAppointment(barberId, barberName) {
  ...
  if (state.currentUser.booking) {
    utils.showAlert('لديك حجز بالفعل، لا يمكنك الحجز أكثر من مرة في نفس الوقت', 'error');
    return;
  }
  ...
  try {
    ...
    utils.showAlert(`تم الحجز بنجاح مع الحلاق ${barberName}`);
  } catch (error) {
    utils.showAlert('حدث خطأ أثناء الحجز: ' + error.message, 'error');
...
async function cancelBooking() {
  ...
  try {
    ...
    utils.showAlert('تم إلغاء الحجز بنجاح');
  } catch (error) {
    utils.showAlert('حدث خطأ أثناء إلغاء الحجز: ' + error.message, 'error');
...
async function submitRating() {
  ...
  try {
    ...
    utils.showAlert('حدث خطأ أثناء إرسال التقييم', 'error');
  }
}
...
function setupBarberRatingStars() {
  elements.barber.ratingStars.forEach(star => {
    star.addEventListener('click', function() {
      const rating = parseInt(this.getAttribute('data-rating'));
      elements.barber.ratingStars.forEach((s, i) => {
        if (i < rating) {
          s.classList.add('active');
        } else {
          s.classList.remove('active');
      ...
      state.barberRating = rating;
    });
  });
}

async function submitBarberRating() {
  if (!state.barberRating || !state.currentClientToRate) return;
  
  try {
    const ratingData = {
      barberId: state.currentUser.id,
      clientId: state.currentClientToRate.clientId,
      clientName: state.currentClientToRate.clientName,
      rating: state.barberRating,
      comment: elements.barber.ratingComment.value.trim(),
      timestamp: Date.now(),
      ratedBy: 'barber'
    };
    
    await push(ref(database, `barberRatings/${state.currentClientToRate.clientId}`), ratingData);
    await updateBarberClientRating(state.currentClientToRate.clientId);
    
    hideBarberRating();
    utils.showAlert('شكراً لتقييمك للعميل!');
    
  } catch (error) {
    console.error('Barber rating error:', error);
    utils.showAlert('حدث خطأ أثناء إرسال التقييم', 'error');
  }
}

async function updateBarberClientRating(clientId) {
  const ratingsRef = ref(database, `barberRatings/${clientId}`);
  const snapshot = await get(ratingsRef);
  ...
  // Update barber's rating stats
  await update(ref(database, `barbers/${state.currentUser.id}`), {
    barberRating: averageRating.toFixed(1),
    barberRatingCount: totalRatings
  });
}

function hideBarberRating() {
  elements.barber.ratingContainer.classList.add('hidden');
  state.currentClientToRate = null;
  state.barberRating = null;
  elements.barber.ratingStars.forEach(star => star.classList.remove('active'));
  elements.barber.ratingComment.value = '';
}
...
async function completeClient(barberId, bookingId) {
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      // إظهار نموذج التقييم للحلاق
      elements.barber.ratingContainer.classList.remove('hidden');
      elements.barber.ratingStars.forEach(star => star.classList.remove('active'));
      elements.barber.ratingComment.value = '';
      state.barberRating = null;
      
      utils.showAlert('تم إنهاء خدمة العميل بنجاح، يرجى تقييم تجربة العمل معه');
    }
  } catch (error) {
    utils.showAlert('حدث خطأ أثناء إنهاء الخدمة: ' + error.message, 'error');
    console.error('Complete client error:', error);
  }
}

// Barber ratings management
async function loadBarberRatings() {
  if (!state.currentUser || state.currentUser.type !== 'barber') return;
  
  if (state.barberRatingsListener) {
    off(state.barberRatingsListener);
  }
  
  // Load barber's own rating stats
  const barberRef = ref(database, `barbers/${state.currentUser.id}`);
  const barberSnapshot = await get(barberRef);
  if (barberSnapshot.exists()) {
    const barberData = barberSnapshot.val();
    elements.barber.averageRating.textContent = barberData.barberRating || '0.0';
    elements.barber.ratingCount.textContent = `(${barberData.barberRatingCount || 0} تقييمات)`;
    
    // Update stars display
    const starsContainer = elements.barber.averageRating.nextElementSibling;
    if (starsContainer) {
      const stars = starsContainer.querySelectorAll('i');
      const rating = Math.round(parseFloat(barberData.barberRating || 0));
  ...
  const ratingsQuery = query(
    ref(database, `barberRatings`),
    orderByChild('timestamp'),
    limitToLast(5)
  );
  
  state.barberRatingsListener = onValue(ratingsQuery, (snapshot) => {
    const ratings = snapshot.val() || {};
    const ratingsArray = Object.values(ratings)
      .filter(r => r.barberId === state.currentUser.id)
      .sort((a, b) => b.timestamp - a.timestamp);
    
    elements.barber.recentRatings.innerHTML = '';
    
    if (ratingsArray.length === 0) {
      elements.barber.recentRatings.innerHTML = '<p>لا توجد تقييمات حديثة</p>';
      return;
    }
    ...
    ratingsArray.forEach(rating => {
      ...
      ratingItem.innerHTML = `
        <div class="rating-item-header">
          <span class="rating-item-name">${rating.clientName}</span>
      ...
      elements.barber.recentRatings.appendChild(ratingItem);
    });
  }, (error) => {
    console.error('Load barber ratings error:', error);
  });
}
...
async function logout() {
  try {
    ...
    if (state.barberRatingsListener) off(state.barberRatingsListener);
    ...
    state.barberRatingsListener = null;
    state.currentRating = null;
    state.currentClientToRate = null;
    state.barberRating = null;
    ...
    utils.showAlert('حدث خطأ أثناء تسجيل الخروج: ' + error.message, 'error');
